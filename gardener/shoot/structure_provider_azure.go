...
func AzureInfrastructureConfigResource() *schema.Resource {
...
func ExpandAzureInfrastructureConfig(az []interface{}) *corev1beta1.ProviderConfig {
...
func expandNetworks(networks []interface{}) azAlpha1.NetworkConfig {
	...
	if v, ok := in["service_endpoints"].(*schema.Set); ok {
		obj.ServiceEndpoints = lib.ExpandSet(v)
	}
...
func FlattenAzureInfrastructureConfig(in *corev1beta1.ProviderConfig) []interface{} {
	...
	if len(azConfigObj.Networks.ServiceEndpoints) > 0 {
		net["service_endpoints"] = lib.NewStringSet(schema.HashString, azConfigObj.Networks.ServiceEndpoints)
	}
